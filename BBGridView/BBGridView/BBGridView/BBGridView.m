...
@interface BBGridView ()<GridButtonDelegate>
...
-(void)setupUI
{
    ...
    for (NSInteger index = 0; index < self.showGridTitleArray.count; index++) {
        ...
        BBGridButton *gridItem =[[BBGridButton alloc]initWithFrame:CGRectZero title:gridTitle normalImage:normalImage highlightedImage:highlightedImage girdId:gridID atIndex:index isAddDelete:isAddDelete deleteImage:deleteIconImage withIconImageString:gridImage];
...
-(void)gridItemDidDeleteClicked:(UIButton *)deleteButton
{
    ...
    for (NSInteger i = 0; i < _gridListArray.count; i++) {
        BBGridButton *removeGrid = _gridListArray[i];
        if (removeGrid.gridId == deleteButton.tag) {
            ...
            for (NSInteger index = removeGrid.gridIndex; index < count; index++) {
                BBGridButton *preGrid = _gridListArray[index];
                BBGridButton *nextGrid = _gridListArray[index+1];
...
-(void)pressGestureStateBegan:(UILongPressGestureRecognizer *)longPressGesture withGridItem:(BBGridButton *)gird
{
    NSLog(@"长按.........");
...
-(void)pressGestureStateChangedWithPoint:(CGPoint)gridPoint grigItem:(BBGridButton *)girdItem
{
    if (isSelected && girdItem.isChecked) {
        ...
        //移动到目标格子的索引下标
        NSInteger toIndex = [BBGridButton indexOfPoint:girdItem.center withButton:girdItem gridArray:_gridListArray];
        if (toIndex == -1) {
            isOrigin = YES;
        }else
        {
        ...
        if (toIndex < 0 || toIndex >= borderIndex) {
            ...
            //获取移动到目标格子
            BBGridButton *targetGrid = _gridListArray[toIndex];
            ...
            if ((fromIndex - toIndex) > 0) {
                ...
                for (NSInteger i = toIndex; i < fromIndex; i++) {
                    BBGridButton *lastGrid = _gridListArray[lastGridIndex];
                    BBGridButton *preGrid = _gridListArray[lastGridIndex-1];
                    [UIView animateWithDuration:0.5 animations:^{
                ...
                for (NSInteger i = fromIndex; i < toIndex; i++) {
                    BBGridButton *topOneGrid = _gridListArray[i];
                    BBGridButton *nextGrid = _gridListArray[i+1];
                    //实时更新格子的索引下标
...
-(void)pressGestureStateEnded:(BBGridButton *)gridItem
{
    NSLog(@"拖动格子结束.........");
...
- (void)sortGridList
{
    ...
    [_gridListArray sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
        BBGridButton *tempGrid1 = (BBGridButton *)obj1;
        BBGridButton *tempGrid2 = (BBGridButton *)obj2;
        return tempGrid1.gridIndex > tempGrid2.gridIndex;
    }];
    
    //更新所有格子的中心点坐标信息
    for (NSInteger i = 0; i < _gridListArray.count; i++) {
        BBGridButton *gridItem = _gridListArray[i];
...
#pragma mark ---点击 按钮----
- (void)itemAction:(BBGridButton *)item
{
